...
(function () {
  ...
  async function loadRaidAttackLogs(report) {
    ...
    while (next && pages < 100) {
      const data = await apiRequest(next, { cacheKey: `raid-attacks:${report?.id || from}:${next}`, cacheMs: 10 * 60 * 1000 });
    ...
    if (next) throw new Error("Attack-log pagination exceeded the local safety limit. Narrower raid data is required.");
    return all;
  }
