...
(function () {
  ...
  async function loadRaidAttackLogs(report) {
    const from = int(report?.start, 0);
    const to = int(report?.end, 0);
    if (!from || !to || to <= from) throw new Error("The selected raid does not have a valid completed time range.");

    let next = `/faction/attacks?filters=outgoing&limit=100&sort=ASC&from=${from}&to=${to}`;
    const all = [];
    const seenAttacks = new Set();
    const seenPageLinks = new Set();
    let pages = 0;
    let stalledPages = 0;

    // Do not impose a fixed page count. Large/long raids can legitimately contain
    // more than 10,000 outgoing faction attacks. Instead, stop only when Torn says
    // there is no next page, or if Torn returns a repeated/stalled pagination cursor.
    while (next) {
      const pageLink = String(next);
      if (seenPageLinks.has(pageLink)) {
        throw new Error(`Torn returned a repeated attack-log pagination link after ${pages} page${pages === 1 ? "" : "s"}. RRH stopped to prevent an infinite request loop.`);
      }
      seenPageLinks.add(pageLink);

      const data = await apiRequest(pageLink, {
        cacheKey: `raid-attacks:${report?.id || from}:${pageLink}`,
        cacheMs: 30 * 60 * 1000,
      });
      ...
      for (const attack of batch) {
        const id = String(attack?.id || attack?.code || `${attack?.started}-${attack?.attacker?.id}-${attack?.defender?.id}`);
      ...
      pages += 1;
      const nextLink = data?._metadata?.links?.next || null;

      // A valid Torn page should either add new rows or finish pagination. Allow a
      // couple of odd/empty pages, but never spin forever on a changing bad cursor.
      if (nextLink && addedThisPage === 0) stalledPages += 1;
      else stalledPages = 0;
      if (stalledPages >= 3) {
        throw new Error(`Torn attack-log pagination stopped making progress after ${pages} pages. RRH stopped to prevent repeated API requests.`);
      }

      if (pages === 1 || pages % 5 === 0 || !nextLink) {
        setStatus(`Reading raid attack logs: ${pages} page${pages === 1 ? "" : "s"}, ${all.length.toLocaleString()} unique outgoing attack${all.length === 1 ? "" : "s"} loaded. RRH will continue until Torn reports the final page...`);
      }
